...
function setValid(elem, valid) {
  // подсветка неправильно заполненых полей красным
...
function setValidFast(element, valid) {
  // подсветка неправильно заполненых полей красным
  //console.log('setValid', valid, elem);
...
function openMenu(menuButtonId, menuContainerId, menuItemActions = []) {
  ...
  menuButton.off('click').on('click', function (event) {
    ...
    console.log(
      'buttonOffset, buttonHeight, buttonWidth, menuWidth',
      buttonOffset,
      buttonHeight,
      buttonWidth,
      menuWidth
